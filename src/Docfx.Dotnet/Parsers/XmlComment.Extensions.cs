...
internal partial class XmlComment
{
    ...
    private IEnumerable<string> GetMultipleExampleNodes(XNode rootNode, string selector)
    {
        // Gets nodes with XPath selector
        var nodes = rootNode.XPathSelectElements(selector).ToArray();
        foreach (var node in nodes)
        {
            yield return GetXmlValue(node);
        }
    }
    ...
    private static string GetMarkdownText(XElement elem)
    {
        ...
        foreach (var node in nodes)
        {
            if (NeedEmptyLineBefore(node))
                node.AddBeforeSelf(new XText("\n"));

            if (NeedEmptyLineAfter(node))
    ...
    private static bool NeedEmptyLineBefore(XElement node)
    {
    ...
    private static bool NeedEmptyLineAfter(XElement node)
    {
...
file static class XmlElementExtensions
{
    ...
    public static string GetInnerXml(this XElement elem)
    {
        ...
        // Trim emding spaces/lines if starts with HTML tag.
